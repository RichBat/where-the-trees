...
def _get_orchard_info(orchard_id: int) -> dict:
    ...
    if response.status_code == 200:
        ...
        if len(response_data) == 0:
            raise ValueError('This Orchard ID has no surveys')
        return {'status': response.status_code, 'id': response_data[0]['id']}
    else:
...
def _get_tree_information(survey_id: int) -> dict:
    ...
    if response.status_code == 200 and missing_tree_response.status_code == 200:
        response_data = response.json()['results']
        if len(response_data) == 0:
            raise ValueError("There are no trees in the survey data.")
        missing_tree_count = missing_tree_response.json()
        if "missing_tree_count" not in missing_tree_count:
            raise ValueError("Missing tree count not present.")
        return {'status': response.status_code,
                'trees': response_data,
                'missing': missing_tree_count["missing_tree_count"]}
    else:
        bad_status = response.status_code if response.status_code != 200 else missing_tree_response.status_code
        raise ValueError(f'Error code: {bad_status}, either the survey tree list or missing tree counts are not available.')


def get_aero_tree_info(orchard_id) -> Tuple[dict, dict]:
    """
